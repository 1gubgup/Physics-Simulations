...
public class FlagController : MonoBehaviour
{
    ...
    //ComputeShader对象定义
    public ComputeShader flagSimulate;
    //ComputeBuffer定义
    ...
    public void Initialize()
    {
        ...
        //设置系统参数
        flagSimulate.SetVector("springKs", springKs);
        flagSimulate.SetFloat("mass", mass);
        flagSimulate.SetFloat("dt", stepTime);

        //计算顶点规模及弹簧原长
        distance = (float)sizeX / (float)(_vertexCountX - 1);
        flagSimulate.SetInts("size", _vertexCountX, _vertexCountY, _totalVertexCount);
        flagSimulate.SetVector("restLengths", new Vector3(distance, distance * Mathf.Sqrt(2), distance * 2));

        //绑定kernel函数
        _initKernel = flagSimulate.FindKernel("Init");
        _stepKernel = flagSimulate.FindKernel("Step");
        ...
        {
            flagSimulate.SetBuffer(k, "velocities", _velocityBuffer);
            flagSimulate.SetBuffer(k, "positions", _positionBuffer);
        };
        setBufferForKernel(_initKernel);
        setBufferForKernel(_stepKernel);

        //执行init kernel
        flagSimulate.Dispatch(_initKernel, _groupX, _groupY, 1);
    ...
    void Update()
    {
        ...
        {
            //执行step kernel
            flagSimulate.Dispatch(_stepKernel, _groupX, _groupY, 1);
