...
public class PendulumTrpz : MonoBehaviour
{
    ...
    void UpdatePosition()
    {
        ...
        // 2. calculate values after a time step
        float newTheta = lastTheta + lastOmega * Time.deltaTime * step;
        float newOmega = lastOmega - (g / l) * Mathf.Sin(lastTheta) * Time.deltaTime * step;
        // 3. update theta
        theta = lastTheta + (lastOmega + newOmega) / 2 * Time.deltaTime * step;
        // 4. update omega
        omega = lastOmega - (g / l) * Mathf.Sin((lastTheta + newTheta) / 2) * Time.deltaTime * step;
