...
public class UI_Step : MonoBehaviour {
    ...
    public void updateFromBigStep(int stepID)
    {
        ...
        if (curBigStepID != 0)
        {
            bigSteps[curBigStepID-1].transform.Find("Locate").gameObject.SetActive(false);
            foreach (GameObject smallStep in smallSteps[curBigStepID - 1])
            {
                smallStep.transform.Find("Locate").gameObject.SetActive(false);
        ...
        curBigStepID = stepID;
        curSmallStepID = 0;
        bigSteps[curBigStepID - 1].transform.Find("Locate").gameObject.SetActive(true);
    ...
    public void updateFromSmallStep(int stepID)
    {
        ...
        //Debug.Log(bigSteps[stepID - 1].GetComponent<UI_BigStep>().stepTitle);
        if (curSmallStepID != 0) {
            smallSteps[curBigStepID - 1][curSmallStepID - 1].transform.Find("Locate").gameObject.SetActive(false);
        }
        ...
        smallSteps[curBigStepID - 1][curSmallStepID - 1].transform.Find("Locate").gameObject.SetActive(true);
    ...
    public void deleteBigStep(int stepID)
    {
        for (int i = stepID; i < bigSteps.Count; i++)
        {
            bigSteps[i].GetComponent<UI_StepContent>().stepID--;
            bigSteps[i].GetComponentInChildren<Text>().text = "流程" + i;
        }
        Destroy(bigSteps[stepID - 1]);
        foreach(GameObject smallStep in smallSteps[stepID - 1])
        {
            Destroy(smallStep);
        }
        bigSteps.RemoveAt(stepID - 1);
        smallSteps.RemoveAt(stepID - 1);
        Debug.Log(curBigStepID + " " + stepID);
        if(curBigStepID >= stepID)
        {
            if (curBigStepID == stepID)
            {
                curBigStepID = curSmallStepID = 0;
                bigTitleInputText.GetComponent<InputField>().interactable = false;
                smallTitleInputText.GetComponent<InputField>().interactable = false;
                eventNameDropdown.GetComponent<Dropdown>().interactable = false;
                eventTypeDropdown.GetComponent<Dropdown>().interactable = false;
            }
            else updateFromBigStep(--curBigStepID);
        }
    }

    public void deleteSmallStep(int stepID)
    {
        for (int i = stepID; i < smallSteps[curBigStepID - 1].Count; i++)
        {
            smallSteps[curBigStepID - 1][i].GetComponent<UI_StepContent>().stepID--;
            smallSteps[curBigStepID - 1][i].GetComponentInChildren<Text>().text = "步骤" + i;
        }
        Destroy(smallSteps[curBigStepID - 1][stepID - 1]);
        smallSteps[curBigStepID - 1].RemoveAt(stepID - 1);
        if (curSmallStepID >= stepID)
        {
            if (curSmallStepID == stepID)
            {
                curSmallStepID = 0;
            ...
            else updateFromSmallStep(--curSmallStepID); 
        }
    }
