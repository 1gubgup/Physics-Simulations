        ...
        void EditObject()
        {
            ...
            if (Physics.Raycast(ray, out hit, 1000))
            {
                ...
                else if(hit.transform.tag == "Text")
                {
                    ...
                    textEditor.transform.position = Input.mousePosition + new Vector3(0.5f * textEditor.GetComponent<RectTransform>().sizeDelta.x, -0.5f * textEditor.GetComponent<RectTransform>().sizeDelta.y, 0);
                    text = hit.transform.GetChild(0).gameObject;
                    foreach (UI.UI_Slider slider in textEditor.GetComponentsInChildren<UI.UI_Slider>())
                    {
                        slider.targetText = text;
                    }
                    textSizeSlider.GetComponent<Slider>().value = text.transform.localScale.x / text.GetComponent<Lab_Text>().srcScale.x;
                    ...
                    textRSlider.GetComponent<Slider>().value = text.GetComponent<TextMesh>().color.r * 255;
                    textGSlider.GetComponent<Slider>().value = text.GetComponent<TextMesh>().color.g * 255;
                    textBSlider.GetComponent<Slider>().value = text.GetComponent<TextMesh>().color.b * 255;
                    screen.SetActive(true);
                    Camera.main.gameObject.GetComponent<Lab.Lab_Controller>().enabled = false;
                }
            }
